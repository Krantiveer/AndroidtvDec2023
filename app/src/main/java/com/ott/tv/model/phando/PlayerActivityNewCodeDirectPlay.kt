...
class PlayerActivityNewCodeDirectPlay : AppCompatActivity(), ControllerVisibilityListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_player_direct_play)
...
*/
        ...
        videoType = this.intent.getStringExtra("type").toString()
        id = this.intent.getStringExtra("video_id").toString()
        title = this.intent.getStringExtra("title").toString()
      //  this.intent.getStringExtra("title").toString()
        ...
        if (videoType == "M") {
            if (id != null) {
                getData(videoType!!, id)
            } else {
                getData(videoType!!, id)
            }
        } else if (videoType == "T") {
        //    getDataTvseries(videoType!!, videoId)
        }
        ...
        call.enqueue(object : Callback<MediaplaybackData?> {
            ...
            //    activityIndicator(false)
                if (response.code() == 200 && response.body() != null) {
                    ...
                    DetailsActivityPhando.singleDetailsRelated = singleDetails
                    ...
                    if (singleDetails!!.list != null) {
                        ...
                        //    tvWatchNow!!.visibility = View.GONE
                          ...
                          //  Log.i(TAG, "intiViews: "+singleDetails!!.list!!.is_live)

                            intiViews(singleDetails!!.list!!.is_live)
                            initVideoPlayer(singleDetails!!.list.media_url, videoType)
                        }
                    ...
                    CMHelper.setSnackBar(
                        this@PlayerActivityNewCodeDirectPlay.currentFocus,
...
*/
    ...
    private fun intiViews(isLive: Int) {
        ...
        movieTitleTV!!.text= singleDetails?.list?.title
...
*/
    ...
    private fun setVideoQuality() {
        @SuppressLint("InflateParams") val builder = AlertDialog.Builder(this@PlayerActivityNewCodeDirectPlay)
        val view = LayoutInflater.from(this@PlayerActivityNewCodeDirectPlay)
        ...
        Log.i(
            ...
            "setVideoQuality: " + PreferenceUtils.getInstance()
                .getVideoQualityPref(this@PlayerActivityNewCodeDirectPlay)
        )
        if (PreferenceUtils.getInstance().getVideoQualityPref(this@PlayerActivityNewCodeDirectPlay)
...
*/
    ...
    private fun handleBackPress() {
        ...
        ToastMsg(this@PlayerActivityNewCodeDirectPlay).toastIconSuccess("Please click BACK again to exit.")
    ...
    private fun openServerDialog(videos: List<Video>?) {
        if (videos != null) {
            ...
            val builder = AlertDialog.Builder(this@PlayerActivityNewCodeDirectPlay)
            val view = LayoutInflater.from(this@PlayerActivityNewCodeDirectPlay)
                .inflate(R.layout.layout_server_tv, null)
            val serverRv = view.findViewById<RecyclerView>(R.id.serverRv)
            val serverAdapter = ServerAdapter(this@PlayerActivityNewCodeDirectPlay, videoList, "movie")
            serverRv.layoutManager = LinearLayoutManager(this@PlayerActivityNewCodeDirectPlay)
            serverRv.setHasFixedSize(true)
            ...
            serverAdapter.setOnItemClickListener { view1: View?, obj: Video, position: Int, holder: OriginalViewHolder? ->
                val playerIntent =
                    Intent(this@PlayerActivityNewCodeDirectPlay, PlayerActivityNewCodeDirectPlay::class.java)
    ...
    private fun openSubtitleDialog() {
        if (subtitleList != null) {
            ...
            if (subtitleList!!.size > 0) {
                ...
                val serverRv = view.findViewById<RecyclerView>(R.id.serverRv)
                val adapter = SubtitleListAdapter(this@PlayerActivityNewCodeDirectPlay, subtitleList)
                serverRv.layoutManager = LinearLayoutManager(this@PlayerActivityNewCodeDirectPlay)
                serverRv.setHasFixedSize(true)
...
*/
    ...
    fun initVideoPlayer(url: String?, type: String) {
        Log.i(TAG, "initVideoPlayer: $type"+singleDetails?.list?.is_live)
        if (player != null) {
            player!!.stop()
            player!!.release()
        }
...
*/
           ...
           // player!!.setMediaSource(hlsMediaSource)
            ...
            Log.i(TAG, "initVideoPlayer: $type")
            initYoutubeVideo(url, this@PlayerActivityNewCodeDirectPlay, 18)
        } else {
