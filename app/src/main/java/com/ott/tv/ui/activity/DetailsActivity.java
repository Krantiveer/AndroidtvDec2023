    ...
    private void payAndWatchTV() {
        if (singleDetails != null) {
            ...
            if (tvWatchNow.getText() == null || !tvWatchNow.getText().toString().equalsIgnoreCase("Watch Now")) {
                CMHelper.setSnackBar(this.getCurrentFocus(), "Enjoy Premium ContentWatch anything without ads Watch  Please Subscribe Or Rent  from MOBILE APP | WEBSITE -"+Config.WebsiteURL , 1, 10000);
                return;
