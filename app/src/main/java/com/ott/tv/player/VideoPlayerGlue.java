...
public class VideoPlayerGlue extends PlaybackTransportControlGlue<LeanbackPlayerAdapter> {
    ...
    private static final long TEN_SECONDS = TimeUnit.SECONDS.toMillis(20);
    ...
    public boolean onKey(View v, int keyCode, KeyEvent event) {
        ...
        Log.d("kingsa2","Down time is" + event.getDownTime()+"with action:" + event.getAction()+ "with repeat count"+ event.getRepeatCount()+"with long press"+ event.isLongPress());
        if (action == KeyEvent.KEYCODE_DPAD_RIGHT && event.isLongPress()) {
            Log.d("kingsa2","LOng pres Down time is" + event.getDownTime());
            Log.d("kingsa2", "Inside long press of Dpad center event");
        ...
        switch (keyCode) {
           ...
           *//*     if (!get.isControllerVisible()) {
                    exoPlayerView.showController();
                }
           *//*         Log.e("SPLASH ACTIVITY", "movieIndex : "+isPlaying()+isControlsOverlayAutoHideEnabled() +isSeekEnabled()+isPrepared() );
                handled = true;
                long time = SystemClock.uptimeMillis();
                if (mLastKeyDownTime + mPressedDelta <= time) {
                    Log.e("SPLASH ACTIVITY", "movieIndex : " +time  +"~~~"+mLastKeyDownTime);
                }
